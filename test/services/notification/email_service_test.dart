...
// Custom mock for FunctionsClient
...
void main() {
  ...
  setUp(() {
    ...
    when(
      () => mockFunctionsClient.invoke(any(), body: any(named: 'body')),
  ...
  group('EmailService', () {
    test(
      'sendBookingConfirmationEmail should send a confirmation email',
      () async {
        // Arrange
        ...
        when(
          () => mockUserDatabaseService.getUser(any()),
    ...
    test('sendBookingUpdateEmail should send an update email', () async {
      ...
      verify(
        () =>
            mockFunctionsClient.invoke('send-email', body: any(named: 'body')),
      ).called(1);
    });

    test(
      'sendBookingCancellationEmail should send a cancellation email',
      () async {
        // Arrange
        final booking = Booking(
          id: 'booking123',
