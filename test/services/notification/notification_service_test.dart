...
class MockNotificationDatabaseService extends Mock implements NotificationDatabaseService {}
...
void main() {
  ...
  group('NotificationService', () {
    test('getNotifications returns notifications from database service', () async {
      ...
      when(() => mockDatabaseService.getNotifications(any()))
      ...
      verify(() => mockDatabaseService.getNotifications('test-user-id')).called(1);
    });

    test('getUnreadNotifications returns unread notifications from database service', () async {
      // Create test notifications
      final testNotifications = [
        Notification(
          id: 'notification-1',
      ...
      // Set up mock response
      when(() => mockDatabaseService.getUnreadNotifications(any()))
          .thenAnswer((_) async => testNotifications);
      ...
      // Verify the result
      expect(result, equals(testNotifications));
      verify(() => mockDatabaseService.getUnreadNotifications('test-user-id')).called(1);
    });

    test('markAsRead calls database service', () async {
      // Set up mock response
      when(() => mockDatabaseService.markAsRead(any()))
          .thenAnswer((_) async {});
    ...
    test('markAllAsRead calls database service', () async {
      // Set up mock response
      when(() => mockDatabaseService.markAllAsRead(any()))
          .thenAnswer((_) async {});
    ...
    test('deleteNotification calls database service', () async {
      // Set up mock response
      when(() => mockDatabaseService.deleteNotification(any()))
          .thenAnswer((_) async {});
      ...
      // Verify the database service was called
      verify(() => mockDatabaseService.deleteNotification('notification-1')).called(1);
    });

    test('createBookingConfirmationNotification creates and sends notifications', () async {
      // Create test booking
      ...
      when(() => mockDatabaseService.createNotification(any()))
          .thenAnswer((_) async => 'notification-id');
      when(() => mockMessagingService.sendMessageToUser(
      ...
      // Call the method
      await notificationService.createBookingConfirmationNotification(booking);

      // Verify the database service was called
      verify(() => mockDatabaseService.createNotification(any())).called(1);

      // Verify the messaging service was called
      verify(() => mockMessagingService.sendMessageToUser(
            userId: 'test-user-id',
    ...
    test('createBookingReminderNotification creates and sends notifications', () async {
      // Create test booking
      final booking = Booking(
        id: 'booking-1',
        userId: 'test-user-id',
