...
class ReminderService {
  ...
  Future<void> scheduleBookingReminders(Booking booking) async {
    try {
      ...
      Logger.i(
        'Booking reminders scheduled for booking ${booking.id}',
        tag: 'ReminderService',
      );
    } catch (e) {
      Logger.e(
        'Error scheduling booking reminders: $e',
        tag: 'ReminderService',
      );
    }
  }
  ...
  Future<void> _scheduleBookingReminder(
    ...
    try {
      ...
      final reminderDate = booking.bookingDateTime.subtract(
      ...
      if (reminderDate.isBefore(DateTime.now())) {
        Logger.i(
          'Skipping reminder for booking ${booking.id} as reminder date is in the past',
          tag: 'ReminderService',
        );
        return;
      }
      ...
      await _messagingService.scheduleNotification(
        ...
        body:
            'Your booking for ${booking.serviceName} is in $daysBeforeEvent ${daysBeforeEvent == 1 ? 'day' : 'days'} on ${_formatDate(booking.bookingDateTime)}.',
        scheduledDate: reminderDate,
        payload: booking.id,
      );

      Logger.i(
        'Scheduled reminder for booking ${booking.id} $daysBeforeEvent days before',
        tag: 'ReminderService',
      );
    } catch (e) {
  ...
  Future<void> checkAndSendBookingReminders() async {
    try {
      ...
      final bookings = await _bookingDatabaseService.getUpcomingBookings(
        userId,
      );
      ...
      for (final booking in bookings) {
        ...
        final daysUntilBooking =
            booking.bookingDateTime.difference(DateTime.now()).inDays;
        ...
        if (daysUntilBooking == 7 ||
            ...
            daysUntilBooking == 1) {
          // Create notification
          await _notificationService.createBookingReminderNotification(
            booking,
            daysUntilBooking,
          );
          ...
          Logger.i(
            'Sent reminder for booking ${booking.id} $daysUntilBooking days before',
            tag: 'ReminderService',
          );
        }
      }
      ...
      Logger.e(
        'Error checking and sending booking reminders: $e',
        tag: 'ReminderService',
      );
    }
  }
  ...
  Future<void> cancelBookingReminders(String bookingId) async {
    try {
      ...
      Logger.i(
        'Booking reminders cancelled for booking $bookingId',
        tag: 'ReminderService',
      );
    } catch (e) {
      Logger.e(
        'Error cancelling booking reminders: $e',
        tag: 'ReminderService',
      );
    }
  }
  ...
  Future<void> _cancelBookingReminder(
    String bookingId,
    int daysBeforeEvent,
  ) async {
    try {
      ...
      Logger.i(
        'Cancelled reminder for booking $bookingId $daysBeforeEvent days before',
        tag: 'ReminderService',
      );
    } catch (e) {
